...
Terraform deploys the following components:
   ...
   - Two FortiGate-VM instances with eight NICs
   - Four firewall rules: one for external, one for internal, one for sync, one for HA management, one for hub, one for 3rd party hub(pphub), one for internal2 and one for internal3.
